...
class MainActivity : AppCompatActivity() {
    ...
    var runThread=false // progress Bar
    var thread:ProgressThread?=null // progress Bar
    ...
    inner class ProgressThread:Thread(){
        override fun run() {
            while(runThread){
                binding.progressBar.incrementProgressBy(1000) // 1초 씩 증가
                SystemClock.sleep(1000) // 1초 쉰다
